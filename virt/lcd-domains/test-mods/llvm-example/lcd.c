...
int __noreturn __init llvm_example_lcd_init(void) 
{
	int ret = 0;
	cptr_t chnl;
...
void llvm_example_lcd_exit(void)
{
	/* No-op, and in this simple example, never called. */
	return;
}

/* These static inlines/macros mark the init/exit functions so the
 * module loader (in the real environment) will invoke them when the
 * module is loaded. For SMACK, just ignore these. */
